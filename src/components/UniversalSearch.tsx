...
import { searchTalent } from '../data/talent'
...
export function UniversalSearch({ audience, onOpenJobs, onOpenTalent }: UniversalSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    return {
      ...
      talent: searchTalent(deferredQuery).slice(0, 5),
    }
  }, [deferredQuery])
      ...
      {focused && query.trim() && (
        <>
          ...
          <div className="universal-results">
            ...
            {audience === 'recruiter' && results.talent.length > 0 && <section><p>VERIFIED TALENT</p>{results.talent.map((talent) => <button type="button" key={talent.id} onClick={() => chooseTalent(talent.skills[0])}><span className="search-result-icon search-result-icon--talent"><UserRoundSearch size={15} /></span><div><strong>{talent.name}</strong><small>{talent.headline}</small></div><b>{talent.score}</b></button>)}</section>}
